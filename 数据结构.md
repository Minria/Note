...
void getPos(L,i);//按位查找操作。获取表L中第i个位置的元素的值。
void add(&L,i,e);//插入操作。在表L中的第i个位置上插入指定元素e。
void remove(&L,i,&e);//删除操作。删除表L中第i个位置的元素，并用e返回删除元素的值

...
void ListDelte(&L,i,&e);//删除操作。删除表L中第i个位置的元素，并用e返回删除元素的值
void PrintList(L);//输出操作。按前后顺序输出线性表L的所有元素值。
bool Empty(L);//判空操作。若L为空表，则返回true,否则返回false。
void clear(&L);//销毁操作。销毁线性表，并释放线性表L所占用的内存空间。
```
...
找到右子树中序遍历的最小节点，让该值与cur值进行交换，然后通过上面两个条件删除最小节点

找到左子树中序遍历的最大节点，让该值与cur值进行交换，然后通过上面两个条件删除最大节点
...
已知父亲下标为i，左孩子小标为2*i+1，右孩子下标为2*i+2
...
![Snipaste_2021-12-26_22-52-19](https://gitee.com/wang-fuming/dawning/raw/master/img/202112271258169.png)
